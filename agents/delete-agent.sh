...
function usage() {
  cat <<END >&2
USAGE: $0 [-e env] [-a access_token] [-i agent_id] [-v|-h]
...
declare -r EXPECTED_SCOPE="delete:agents"
...
[[ ${opt_verbose} -eq 1 ]] && echo "DELETE ${AUTH0_DOMAIN_URL}api/v2/agents/${agent_id}" >&2

curl -s --request DELETE \
  -H "Authorization: Bearer ${access_token}" \
  --url "${AUTH0_DOMAIN_URL}api/v2/agents/${agent_id}"

...
function usage() {
    cat <<END >&2
USAGE: $0 [-e env] [-a api_token] [-t team_slug] [-k take] [-n next] [-v|-h]
...
while getopts "e:a:t:k:n:hv?" opt; do
    case ${opt} in
    e) ENV=${OPTARG} ;;
    a) api_token=${OPTARG} ;;
    t) TEAM_SLUG=${OPTARG} ;;
...
declare url="${TEAMS_API_URL}/api/members"

...
USAGE: $0 [-e env] [-a api_token] [-t team_slug] [-i member_id] [-T tenant_ids] [-r roles] [-C client_ids] [-v|-h]
        ...
        -T ids        # Comma-separated tenant UUIDs to grant access to (max 10)
        -r roles      # Comma-separated roles to assign (e.g., owner,editor-users)
        -C ids        # Comma-separated client IDs (only for editor-specific-apps role)
        -h|?          # usage
        -v            # verbose

eg,
     $0 -t my-team -i 'auth0|xxx' -T 538c9e21-e3d5-4ad6-b3d0-352c62369fb0 -r owner
     $0 -t my-team -i 'auth0|xxx' -T uuid1,uuid2 -r editor-users,editor-connections
...
[[ -n "${opt_verbose}" ]] && echo "PUT ${TEAMS_API_URL}/api/members/${member_id}/tenants" >&2

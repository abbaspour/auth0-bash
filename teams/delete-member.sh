...
declare -r EXPECTED_SCOPE="delete:members"
...
[[ ${opt_verbose} -eq 1 ]] && echo "DELETE ${TEAMS_API_URL}/api/members/${team_member_id}" >&2

curl -s --request DELETE \
    -H "Authorization: Bearer ${access_token}" \
    --url "${TEAMS_API_URL}/api/members/${team_member_id}"

echo "Team member ${team_member_id} deleted successfully"

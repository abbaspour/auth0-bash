...
USAGE: $0 [-e env] [-a access_token] [-i id] [-v|-h]
        ...
        -i id       # refresh token id
        -h|?        # usage
        -v          # verbose

eg,
     $0 -i 'i5ZqMxSHG6R6PA-C6JshdQ'
END
    exit $1
}
...
declare refresh_token_id=''

while getopts "e:a:i:hv?" opt; do
    case ${opt} in
    e) source "${OPTARG}" ;;
    a) access_token=${OPTARG} ;;
    i) refresh_token_id=$(urlencode "${OPTARG}") ;;
    v) set -x;;
    h | ?) usage 0 ;;
    *) usage 1 ;;
    esac
...
declare -r EXPECTED_SCOPE="read:refresh_tokens"
[[ " $AVAILABLE_SCOPES " == *" $EXPECTED_SCOPE "* ]] || { echo >&2 "ERROR: Insufficient scope in Access Token. Expected: '$EXPECTED_SCOPE', Available: '$AVAILABLE_SCOPES'"; exit 1; }

[[ -z ${refresh_token_id} ]] && { echo >&2 "ERROR: no 'refresh_token_id' defined"; usage 1; }

declare -r AUTH0_DOMAIN_URL=$(jq -Rr 'split(".") | .[1] | @base64d | fromjson | .iss' <<<"${access_token}")

curl -s --get -H "Authorization: Bearer ${access_token}" -H 'content-type: application/json' \
    "${AUTH0_DOMAIN_URL}api/v2/refresh-tokens/${refresh_token_id}" | jq .
